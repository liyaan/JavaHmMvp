...
public class WheelAmPmPicker extends WheelPicker<String> {
    ...
    public WheelAmPmPicker(Context context, AttrSet attrs) {
        super(context, attrs);
    }
    ...
    protected String initDefault() {
        if (dateHelper.getHour(dateHelper.today(), true) >= SingleDateAndTimeConstants.MAX_HOUR_AM_PM) {
            return getContext().getString(ResourceTable.String_picker_pm);
        } else {
            return getContext().getString(ResourceTable.String_picker_am);
        }
    }
    ...
    protected List<String> generateAdapterValues(boolean showOnlyFutureDates){
        return Arrays.asList(
                getContext().getString(ResourceTable.String_picker_am),
                getContext().getString(ResourceTable.String_picker_pm)
        );
    }
    ...
    protected String getFormattedValue(Object value) {
        if (value instanceof Date) {
            ...
            return getLocalizedString(instance.get(Calendar.AM_PM) == Calendar.PM ? ResourceTable.String_picker_pm : ResourceTable.String_picker_am);
        }
