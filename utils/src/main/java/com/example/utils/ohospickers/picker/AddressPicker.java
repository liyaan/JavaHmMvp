...
import static com.example.utils.ohospickers.common.BaseDialog.WRAP_CONTENT;
...
public class AddressPicker extends LinkagePicker {
  ...
  public AddressPicker(Ability activity, ArrayList<Province> provinces) {
...
//        super.makeCenterView();
    ...
    provinceView.setLayoutConfig(new DirectionalLayout.LayoutConfig(provinceWidth, WRAP_CONTENT));
    ...
    if (hideProvince) {
      provinceView.setVisibility(HIDE);
    }
    layout.addComponent(provinceView);
    ...
    cityView.setLayoutConfig(new DirectionalLayout.LayoutConfig(cityWidth, WRAP_CONTENT));

    layout.addComponent(cityView);
    ...
    countyView.setLayoutConfig(new DirectionalLayout.LayoutConfig(countyWidth, WRAP_CONTENT));
    if (hideCounty) {
      countyView.setVisibility(HIDE);
    }
    layout.addComponent(countyView);
...
//                        return;
//                    }
        LogUtils.info(TAG, "change cities after province wheeled");
...
//                        return;
//                    }
        LogUtils.info(TAG, "change counties after city wheeled");
        selectedThirdIndex = 0;//重置县级索引
        //根据地市获取区县
        List<String> counties = provider.provideThirdData(selectedFirstIndex, selectedSecondIndex);
        if (counties.size() > 0) {
