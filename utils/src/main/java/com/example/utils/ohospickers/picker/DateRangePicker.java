...
import static com.example.utils.ohospickers.common.Constants.Public.NONE;
...
import static ohos.agp.utils.LayoutAlignment.RIGHT;
...
public class DateRangePicker extends WheelPicker {
  ...
  public DateRangePicker(Ability activity) {
    this(activity, YEAR_MONTH_DAY, HOUR_24);
  }

  public DateRangePicker(Ability activity, int timeMode) {
    this(activity, YEAR_MONTH_DAY, timeMode);
  }

  public DateRangePicker(Ability activity, int dateMode, int timeMode) {
    super(activity);
...
//    }
  ...
  protected Component makeCenterView() {
    Component view = LayoutScatter.getInstance(getContext()).parse(ResourceTable.Layout_layout_date_range_picker, null, false);
    final WheelView yearView = (WheelView) view.findComponentById(ResourceTable.Id_wv_year);
    final WheelView startMonthView = (WheelView) view.findComponentById(ResourceTable.Id_wv_start_month);
    final WheelView startDayView = (WheelView) view.findComponentById(ResourceTable.Id_wv_start_day);
    final WheelView endMonthView = (WheelView) view.findComponentById(ResourceTable.Id_wv_end_month);
    final WheelView endDayView = (WheelView) view.findComponentById(ResourceTable.Id_wv_end_day);
    final WheelView lineView = (WheelView) view.findComponentById(ResourceTable.Id_wv_line);
